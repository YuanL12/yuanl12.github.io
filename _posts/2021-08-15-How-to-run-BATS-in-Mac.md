...
> **Error**:
> If you use `g++ -o hello_omp.out hello_omp.cpp -fopenmp` in Mac and get an error like `clang: error: unsupported option '-fopenmp'`, this is because g++ in Mac is default to use clang (e.g., `Apple clang version 12.0.0 (clang-1200.0.32.29)` in my case).

The bad thing is Mac use `clang` as an alias for `g++`, and using OpenMP is hard in `clang`. You will need Homebrew to install GCC. The details of installation is listed below:
...
There are two options from now on:
1. Change setting in `~/.zshrc` for a user-wide setting. Because calling a compiler your bash will look into `/usr/bin` by default and not in `/usr/local/bin`, you can add this directory to your `$PATH`. This can be easily done with the command:
...
```
2. You are also able to export a sesstion environment varible by `export CXX=/usr/local/bin/g++-10` in terminal. However, next time if you reopen a terminal and want to use g++ from GNU instead of clang, make sure you change the environment variable again.
